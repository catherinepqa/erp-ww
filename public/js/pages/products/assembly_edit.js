$(function () {
    ...
    var App = {
        ...
        bindEvents: function () {
            ...
            this.$save_top.on('click', this.updateItem);
            this.$save_bottom.on('click', this.updateItem);
                ...
                $.ajax({
                    ...
                    success : function(data) {
                        swal({
                            ...
                            window.location.href = App.baseUrl + "view_assembly/" + $('#item-id').val(); 
                        });
                    },
                });
            });        
        ...
        inventoryItems : function() {
            ...
            $('#tblloc tbody tr').each(function(row, tr){
                ...
                var systemID = $(cols[0]).text();
                var locationID = $(cols[1]).text();
                var buildPoint = $('#bp-'+$(cols[1]).text()).val();
                var preferredStockLevel = $('#psl-'+$(cols[1]).text()).val();
                var defaultReturnCost = $('#drc-'+$(cols[1]).text()).val();
        ...
        priceLevels : function() {
            ...
            $('#tblpl tbody tr').each(function(row, tr){
                var cols = $(tr).children('td');
                var amount = $('#pl-'+$(cols[1]).text()).val();

                if (amount != '') {
                    dt.push({
                        "systemID" : $(cols[0]).text(),
                        "currencyID" : $(cols[1]).text(),
                        "amount" : amount
                    });
                }
            });
...
function deleteBOM(evt){
    var cols = $(evt).closest('tr').children("td");
