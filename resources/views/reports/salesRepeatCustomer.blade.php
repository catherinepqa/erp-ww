...
@section('title', 'Sales by Repeat Customer Country (Gross Amt)')
...
@section('breadcrumb')
    <div class="col-md-6 col-sm-12">
        <h1>Sales by Repeat Customer Country (Gross Amt) Report</h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">Home</li>
                <li class="breadcrumb-item">Reports</li>
                <li class="breadcrumb-item active">Sales by Repeat Customer Country (Gross Amt)</li>
            </ol>
        </nav>
    </div>
@endsection
                ...
                <div class="row" style="margin-top: 3%;">
                    <div class="col-lg-12">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover dataTable salesRepeatCustomer">
                                <thead>
                                    <tr>
                                        <th>COUNTRY</th>
                                        <th>CUSTOMER</th>
                    ...
                    <div class="hidden_values" style="display: none">
                        @include('report_partials._printSalesRepeatCustomer');
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function filter()
    {
        ...
        $.ajax({
            url: "{{ action('ReportsController@filterSalesRepeatCustomer') }}",
            method: 'GET',
            dataType: "json",
            ...
            success: function (data) {
                if (data.salesRepeat != '') {
                    ...
                    $.each(data.salesRepeat, function (key, value) {
                        stock.push(
                            '<tr class="newTr">\n' +
                            '<td> </td>\n' +
                    ...
                    $('.salesRepeatCustomer tbody').append(stock);
                } else {
                    $('.newTr').remove();
                }
            },
    ...
    function exportReport(btn)
    {
        ...
        $.ajax({
            url: "{{ action('ReportsController@exportSalesRepeatCustomer') }}",
            method: 'GET',
            xhrFields: {
                responseType: 'blob'
            },
